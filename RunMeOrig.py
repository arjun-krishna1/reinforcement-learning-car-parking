...
def main():
    ...
    actor_state = (case.x0, case.y0, case.theta0)
    for i in range(TIMESTEPS):
        actor_state = (actor_state[0] + i, actor_state[1] + i, actor_state[2])
        # plt.arrow(actor_state[0], actor_state[1], np.cos(actor_state[2]), np.sin(actor_state[2]), width=0.2, color = "gold")
        temp = case.vehicle.create_polygon(case.x0 + i, case.y0, case.theta0)
        plt.plot(temp[:, 0], temp[:, 1], linestyle='--', linewidth = 0.4, color = 'green')
        fig.canvas.draw()
